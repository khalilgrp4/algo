 ...
 *  
 *   
 * @Khalil <----- !!!!!!! renseignez votre nom ici !!!!!
 * 
 *  
 */
...
public class Vol {
	...
	public boolean estUnTour(){
		...
		if (tableCoordonnees[0].equals(tableCoordonnees[tableCoordonnees.length-1]))
			return true;
		return false;
	}
	...
	public Coordonnees lieuLePlusAuSud() {
		long plusPetiteLatitudde = 1000;
		int indexPlusPetitelatitude = 0;
		for (int i = 0; i < tableCoordonnees.length; i++) {
			if (tableCoordonnees[i].getLatitude()<plusPetiteLatitudde){
				indexPlusPetitelatitude = i;
				plusPetiteLatitudde = tableCoordonnees[i].getLatitude();
			}
		}
		return tableCoordonnees[indexPlusPetitelatitude];
	}
	...
	public Coordonnees lieuLePlusEloigne(){
		...
		double distanceLaPlusEloigne = 0;
		int index = 0;
		for (int i = 1; i < tableCoordonnees.length; i++) {
			if (tableCoordonnees[0].distance(tableCoordonnees[i]) > distanceLaPlusEloigne){
				distanceLaPlusEloigne = tableCoordonnees[0].distance(tableCoordonnees[i]);
				index = i;
			}
		}
		return tableCoordonnees[index];
	}
	...
	public int nombreCiblesAtteintes(Coordonnees[] cibles){
		...
		for (int i = 0; i < cibles.length; i++) {
			if (cibleAtteinte(cibles[i])){
				nbrciblesAtteintes += 1;


			}

		}
		return nbrciblesAtteintes;
	}
	...
	public double distance() {
		...
		for (int i = 0; i < tableCoordonnees.length; i++) {
			if (index < (tableCoordonnees.length-1)){
				distance += tableCoordonnees[i].distance(tableCoordonnees[index]);
				index ++;
			}
